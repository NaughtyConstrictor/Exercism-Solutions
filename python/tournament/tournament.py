...
class AddUpdateFunctions(type):
    def __init__(cls, name, bases, attrs):
        super().__init__(name, bases, attrs)
        cls._make_update_functions()


@functools.total_ordering
class TeamStats(metaclass=AddUpdateFunctions):
    ...
    @classmethod
    def _make_update_functions(cls):
        for name in ["win", "loss", "draw"]:
            attr = name + ("es" if name[-1] == "s" else "s")
            def temp(self, attr=attr):
                setattr(self, attr, getattr(self, attr) + 1)
            temp.__name__ = name
            temp.__doc__ = f"Updates {attr}."
